...
def count():
    ...
    def parse_zips():
        # Dictionary to aggregate counts and currency-specific notional sums by date
        date_aggregates = {}
        # Start with a set of known currencies, but allow dynamic expansion
        ...
        for index, zip_file in enumerate(zip_files, 1):
            ...
            try:
                with ZipFile(zip_file, 'r') as zip_ref:
                    ...
                    with zip_ref.open(csv_filename) as csv_file:
                        df = pd.read_csv(csv_file, low_memory=False)
                        # Check if required columns exist (M: 12, T: 19, V: 21)
                        ...
                        # Convert notional amounts to numeric, coercing errors to NaN
                        ...
                        for date, notional, currency in zip(column_m, column_t_numeric, column_v):
                            if pd.notna(date) and isinstance(date, str) and len(date) == 10:  # Validate date format YYYY-MM-DD
                                try:
                                    datetime.strptime(date, '%Y-%m-%d')  # Check if it's a valid date
                                    if date not in date_aggregates:
                                        date_aggregates[date] = {'count': 0, 'notional': defaultdict(float)}
                                    date_aggregates[date]['count'] += 1
                                    if pd.notna(notional):
                                        currency = str(currency).strip().upper()  # Normalize currency
                                        date_aggregates[date]['notional'][currency] += notional
                                        all_currencies.add(currency)  # Add to set of all currencies
                                except ValueError:
                        ...
                        files_processed += 1
                        print(f"Processed {zip_file}. Current aggregates: {len(date_aggregates)} unique dates.")
            except Exception as e:
                logging.error(f"Error processing {zip_file}: {e}")
                print(f"Error occurred while processing {zip_file}. Continuing to next file.")
        ...
        if date_aggregates:
            ...
            for date, data in date_aggregates.items():
                ...
                row.update({currency: data['notional'][currency] for currency in all_currencies})
                result_data.append(row)
            result_df = pd.DataFrame(result_data)
            # Ensure all currency columns are present, filling missing with 0
            for currency in all_currencies:
                if currency not in result_df.columns:
                    result_df[currency] = 0
            # Reorder columns: Count, Date, then all currencies
            column_order = ['Count', 'Date'] + sorted(list(all_currencies))  # Sort for consistency
            result_df = result_df[column_order]
            total_count = result_df['Count'].sum()
            total_notional_by_currency = {c: result_df[c].sum() for c in all_currencies}
            ...
            result_df = pd.DataFrame(columns=['Count', 'Date'] + sorted(list(all_currencies)))
            print("No valid data found.")
            logging.info("No valid data found for aggregation.")
        
        return result_df

    print("Press Enter to calculate the count of column M dates and notional sums by currency from column T, or type 'q' to quit.")
    user_input = input()
    if user_input.lower() != 'q':
        result_df = parse_zips()
        if not result_df.empty:
            master_csv_path = os.path.join(EQUITY_SOURCE_DIR, 'date_notional_summary_by_all_currencies.csv')
            result_df.to_csv(master_csv_path, index=False)
            print(f"\nSaving results to: {master_csv_path}")
...
if __name__ == "__main__":
    ...
    if query == '6':
        ...
        search_for_swaps = input("Searching for swaps? (s for swaps, enter for other positions): ").lower() == 's'
        verbose = input("Enable verbose mode? (y/n): ").lower() == 'y'
        search_ncen(search_keyword, verbose=False, search_ncen_swaps=False)
